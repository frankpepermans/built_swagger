...
class SwaggerGenerator extends Generator {
  ...
  Future<String> generate(Element element, _) async {
    if (element is ClassElement) {
      ...
      buffer.writeln('''import 'package:logging/logging.dart' show Logger;''');
      buffer.writeln(
      ...
      data.bundles.forEach((Bundle bundle) {
        ...
        buffer.writeln('''final Logger _log = new Logger('$bundleClassName');''');
        buffer.writeln();
        buffer.writeln(
      ...
      flatList.forEach((_PathPart pathPart) {
        ...
        pathPart.next.forEach((_PathPart nextPathPart) {
          ...
          if (nextPathPart.hasOperation) {
            ...
            List<Parameter> otherParameters = nextPathPart.operation.parameters
                .where((Parameter parameter) => parameter.location != 'path')
                .toList(growable: false);
            ...
            buffer.writeln('.then((http.Response response) => response.body)');
  ...
  String _toOperationMethodName(Operation operation) {
    List<String> parts =
        operation.method.split('Using${operation.name.toUpperCase()}');

    return '${operation.name.toLowerCase()[0]}${parts.first[0].toUpperCase()}${parts.first.substring(1)}';
  }
